...
{   
    ...
    if(user.find(username) != user.end() && user[username] == password)
    {
        ...
        if(userConn.find(username) != userConn.end())
        {
            string msg = "User already logged in from IP: " + userConn[username];
            strcpy(buff, msg.c_str());
            send(sock, buff, sizeof(buff), 0);
        }
        else
        {
            ...
            // Forward the login command to the sub-tracker (if necessary)
            strcpy(buff, s.c_str());
            send(subTrackerSockNum, buff, sizeof(buff), 0);
            // pushCommand(s);
            // cout << buff;
        ...
        else if(s.substr(0,14)=="accept_request")
{
    ...
    if (pending.find(grpid) != pending.end()) // Check if group exists in the pending list
    {
        ...
        if (!group[grpid].empty() && group[grpid][0] == username)
        {
            ...
            {
                string msg = "The specified user does not exist.";
                strcpy(buff, msg.c_str());
                send(sock, buff, sizeof(buff), 0);
                return; // Exit early
            }

            // Check if the user has sent a request to join the group
            if (memList.find(userid) != memList.end())
            {
                group[grpid].push_back(userid); // Add user to the group
                pending[grpid].erase(userid);  // Remove the user from pending requests

                // Remove group from pending if no requests remain
            ...
            {
                string msg = "No request from the selected user!";
                strcpy(buff, msg.c_str());
                send(sock, buff, sizeof(buff), 0);
            }
        }
    ...
    {
        string msg = "Group does not exist in the pending list!";
        strcpy(buff, msg.c_str());
        send(sock, buff, sizeof(buff), 0);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    //cout<<"trackerinfo second is: "<<trackerInfo[tnum].second<<"\n";
    PORT=stoi(trackerInfo[tnum-1].second);
    cout<<"port is: "<<PORT<<"\n";
    HOST=trackerInfo[tnum-1].first;
    cout<<PORT<<" "<<HOST<<endl;
